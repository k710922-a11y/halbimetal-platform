...
- Member Hub 곡 카드의 재생 버튼 아래에 TAB 버튼을 추가해 파트별 악보 미리보기 팝업을 열도록 구현
- 악기 파트(기타·베이스·드럼·키보드·보컬·밴드스코어) 카테고리 탭과 미리보기·내려받기를 제공하는 공용 팝업 `src/tab-viewer.js`
- Admin 곡 목록에 TAB 관리 패널을 추가해 곡마다 파트를 지정한 악보 등록·미리보기·삭제 지원
- 악보 파트 정의와 저장소 경로 규칙을 담은 `src/tabs.js`, 운영 안내 문서 `docs/TAB-SHEETS.md`

### Changed

- Member Hub 편곡/자작곡 세션을 TBD 자리표시자에서 실제 곡 목록(재생·TAB·페이지네이션)으로 교체
- Admin 연습곡 세션 선택에 `편곡/자작곡` 추가

### Removed

- Admin 곡 등록 폼의 앨범 커버·타브 악보 업로드 입력 (악보는 곡 목록의 TAB 관리에서만 등록)

- Public Line-up을 보컬·리드 기타·세컨 기타·베이스·키보드·드럼 6인 구성과 전용 멤버 이미지로 개편
- 곡 업로드는 오디오 파일을 저장하지 않고 ID3 및 파일 메타데이터만 목록에 보관하도록 변경
- Member Hub의 BPM과 플레이어를 제거하고 연습곡을 2열, 페이지당 10곡으로 표시
- Admin 합주 일정 저장 완료 피드백 추가 및 Member Hub RSVP 버튼을 카카오톡 안내로 교체
