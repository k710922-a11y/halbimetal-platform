...
- 곡 등록 폼에는 음원만 올립니다. 앨범 커버·악보 업로드 입력은 없앴습니다.
- 악보는 곡 목록의 **TAB 관리** 버튼 → 파트 선택 → 파일 선택 → `악보 등록 · 업로드`.
  같은 패널에서 `미리보기 팝업` 으로 확인하고, `✕` 를 두 번 눌러 한 장씩 삭제할 수 있습니다.

**Member Hub (`/hub.html`)**

- 곡 카드의 재생(▶) 버튼 **바로 아래** 에 `TAB` 버튼이 있습니다. 숫자는 등록된 악보 수입니다.
- 누르면 미리보기 팝업이 열립니다: 위쪽에 악기 파트 카테고리 탭, 가운데에 악보 미리보기,
  아래에 `새 탭에서 열기` 와 `내려받기`. 한 파트에 여러 장이면 페이지 번호로 넘깁니다.
- 보컬 연습/공연곡, Wish List, 편곡/자작곡 세 목록 모두 동일하게 동작합니다.
- 미리보기는 이미지(JPG·PNG·WEBP·GIF·SVG)와 PDF 를 지원합니다. Guitar Pro(gp5 등)처럼
  브라우저가 열 수 없는 형식은 안내 문구와 함께 내려받기만 제공합니다 — 멤버들이 화면에서
  바로 보게 하려면 **PDF 또는 이미지로 올리는 것을 권장**합니다.
- 팝업은 `Esc` 키, 배경 클릭, `✕` 로 닫힙니다.
